...
qa: check-venv pre-commit  ## Quality Assurance (pre-commit + other checks)
...
.PHONY: docker-build docker-push docker-run docker-stop docker-ls

docker-build:  ## Build multi-platform Docker image (arm64 focused)
